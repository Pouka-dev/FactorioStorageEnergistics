...
function DestructionHandlers.OnEntityRemoved(entity)
    local handler = RSE.NodeHandlersRegistry:GetEntityHandler(entity)
    if (handler ~= nil) then
        RSE.NetworksManager.RemoveNodeByEntity(entity)
    end
end
...
function EntityHandlers.OnPasteSettings(event)
    ...
    local handler = RSE.NodeHandlersRegistry:GetEntityHandler(destEntity)
    if (handler ~= nil) then
        local destNode = RSE.NetworksManager.GetNodeForEntity(destEntity)
...
function EntityHandlers.EntityToBlueprintTags(copyEntity, fromTable)
    ...
    local tags = nil
    local handler = RSE.NodeHandlersRegistry:GetEntityHandler(copyEntity)
    if (handler ~= nil) then
        tags = RSE.NetworksManager.GetNodeForEntity(copyEntity)
    end
    return tags
end
