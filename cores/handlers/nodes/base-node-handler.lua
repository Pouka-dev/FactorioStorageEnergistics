...
BaseNodeHandlerController = newclass(Object,function(base,...)
    Object.init(base,...)
    base.HandlerName = ...
    -- Type: Type of handled nodes.
    base.Type = SE.Constants.NodeTypes.Device
    ...
    function BaseNodeHandlerController:Valid()
    ...
    function BaseNodeHandlerController:ExtractPower(watts)
        return 0
    end
    ...
    function BaseNodeHandlerController:StoredPower()
        return 0
    end
    ...
    function BaseNodeHandlerController:InsertItems(stack, simulate)
    end
    ...
    function BaseNodeHandlerController:ExtractItems(stack, simulate)
        return 0
    end
    
    -- GetItemCount( Self, ItemName ) :: int
    -- Returns how many of the item there is in the node
    function BaseNodeHandlerController:GetItemCount(itemName)
        return 0
    end
    
    -- GetPosition( Self ) :: Position{x,y}
    function BaseNodeHandlerController:GetPosition()
    ...
    function BaseNodeHandlerController:GetContents(catalog)
    end
    
    -- GetCircuitNetwork( Self, WireType ) :: LuaCircuitNetwork
    -- Returns the circuit network or nil
    function BaseNodeHandlerController:GetCircuitNetwork(wireType)
        if (BaseNodeHandlerController.Valid(self)) then
    ...
    function BaseNodeHandlerController:IsFiltered(type)
        return false
    end
    ...
    -- type: Filter type, item, fluid, etc
    function BaseNodeHandlerController:GetFilters(type)
        return nil
    end
    
    -- void OnTick(self)
    -- Called when the game ticks, if NeedsTicks is true.
    function BaseNodeHandlerController:OnTick(tick)
    end
    ...
    function BaseNodeHandlerController:OnNetworkTick(network)
    end
    
    -- OnJoinNetwork( Self, Network ) :: void
    -- Called just after node joins an active network.
    function BaseNodeHandlerController:OnJoinNetwork(network)
    end
    
    -- OnLeaveNetwork( Self, Network ) :: void
    -- Called just before the node leaves an network.
    function BaseNodeHandlerController:OnLeaveNetwork(network)
    end
    ...
    function BaseNodeHandlerController:EnsureStructure()
        ...
        self.HandlerName = self.HandlerName or BaseNodeHandlerController.HandlerName
        
        -- Calculate chunk position
        self.ChunkPosition = BaseNodeHandlerController.GetPosition(self)
